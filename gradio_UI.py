...
OLLAMA_SERVER_URL = "http://127.0.0.1:11434"  # "http://172.17.0.2:11434" for docker network
MODEL = "llama3.2"
...
def query_ollama(user_query):
    ...
    try:
        ...
        prompt = f"explain how an attack can happen where {prompt} by refering to {retrieved_files}" #TODO change format of prompt, currently used to test on untrained llm
        print(prompt)
    ...
    payload = {
        "model": "llama3.2",#changed model rag_changes
        "prompt": prompt,
        "stream": True  # Enable streaming for larger responsesb 
        }
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            input = gr.MultimodalTextbox(interactive=True, file_count="single", placeholder="Enter text following the format shown below or upload file")
            with gr.Row():
